...
object ModelCF {
  ...
  def modelCF(sparkSession: SparkSession) = {

    //装载用户评分，该评分由评分器生成(即生成文件personalRatings.txt)

    val personalRatingsPath = "E:\\DATANALYSIS\\src\\main\\resources\\ml-1m\\personalRatings.txt"

    val myRatings = loadRatings(personalRatingsPath)

    val myRatingsRDD = sparkSession.sparkContext.parallelize(myRatings, 1)

    //样本数据目录

    val movielensHomeDir = "E:\\DATANALYSIS\\src\\main\\resources\\ml-1m"

    //装载样本评分数据，其中最后一列Timestamp取除10的余数作为key，Rating为值，即(Int，Rating)

    val ratings = sparkSession.sparkContext.textFile(movielensHomeDir + "\\ratings.dat").map {
    ...
    val movies = sparkSession.sparkContext.textFile(movielensHomeDir + "\\movies.dat").map {
    ...
    val candidates = sparkSession.sparkContext.parallelize(movies.keys.filter(!myRatedMovieIds.contains(_)).toSeq)
